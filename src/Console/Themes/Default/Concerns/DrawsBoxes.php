...
namespace Cosmo\Console\Themes\Default\Concerns;
...
trait DrawsBoxes
{
    ...
    protected function box(
        ...
        $bodyLines = Collection::from(explode(PHP_EOL, $body));
        $footerLines = Collection::from(explode(PHP_EOL, $footer))->filter();
        $width = $this->longest(
            $bodyLines
                ->merge($footerLines)
                ->push($title)
