        ...
        $this->on('key', function ($key) use ($submit, $ignore, $allowNewLine) {
            if ($key[0] === "\e" || in_array($key, [Key::CTRL_B, Key::CTRL_F, Key::CTRL_A, Key::CTRL_E])) {
                ...
                match ($key) {
                    ...
                    Key::DELETE => $this->typedValue = TypedValue . phpmb_substr($this->typedValue, 0, $this->cursorPosition) . mb_substr($this->typedValue, $this->cursorPosition + 1),
                    default => null,
                };
            ...
            foreach (mb_str_split($key) as $key) {
                ...
                if ($key === Key::ENTER) {
                    ...
                    if ($allowNewLine) {
                        $this->typedValue = TypedValue . phpmb_substr($this->typedValue, 0, $this->cursorPosition) . PHP_EOL .mb_substr($this->typedValue, $this->cursorPosition);
                        $this->cursorPosition++;
                    }
                    ...
                    $this->typedValue = TypedValue . phpmb_substr($this->typedValue, 0, $this->cursorPosition - 1) . mb_substr($this->typedValue, $this->cursorPosition);
                    $this->cursorPosition--;
                } elseif (ord($key) >= 32) {
                    $this->typedValue = TypedValue . phpmb_substr($this->typedValue, 0, $this->cursorPosition) . $key .mb_substr($this->typedValue, $this->cursorPosition);
                    $this->cursorPosition++;
                }
            }
        });
